...
cad <- readRDS(glue("/PHI_conf/CancerGroup1/Topics/CancerStatistics/Data/",
  ...
  zap_widths() %>%
  remove_all_labels() %>%
  # GC edit to use FIGO staging for cervical
  mutate(tnm_stage_2 = case_when(
    site10 == "C53" ~ figo_stage_2,
    TRUE ~ tnm_stage_2
  )) %>%
  filter(tnm_stage_2 != "") %>% 
...
# GC edit to left join so cancers where staging info is blank are still included
# These are then rolled into "X"
...
setColWidths(wb, sheet = "Incidence Rates", cols = 1:17, widths = "auto")
