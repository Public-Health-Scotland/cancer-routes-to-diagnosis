...
hb_totals <- joined_data |> 
  count(incidence_year, hb2019name, incidence_type, name = "Cancers")
...
hb_admissions <- joined_data |> 
  count(incidence_year, hb2019name, incidence_type, emergency_flag, 
        name = "Presentations")
...
hb_output <- full_join(hb_totals, hb_admissions) |> 
  mutate(across(where(is.numeric), ~ ifelse(is.na(.), 0, .))) |> 
  mutate(PC_Presentations = Presentations / Cancers,
         Lower_95_CI = Wilson_lowerCI(PC_Presentations, 0.05, Cancers), 
         Upper_95_CI = Wilson_upperCI(PC_Presentations, 0.05, Cancers)) |> 
  mutate(across(c(PC_Presentations, Lower_95_CI, Upper_95_CI), 
                ~ round_half_up(.x * 100, 1))) |> 
  arrange(incidence_type, incidence_year) |> 
  replace_na(list(hb2019name = "Unknown")) |> 
  select(incidence_year, hb2019name, incidence_type, emergency_flag, Cancers, 
         Presentations, PC_Presentations, Lower_95_CI, Upper_95_CI)
...
hb_output <- hb_output |> 
  group_by(incidence_year, hb2019name, emergency_flag) |> 
  summarise(Cancers = sum(Cancers), 
            Presentations = sum(Presentations)) |> 
  mutate(PC_Presentations = Presentations / Cancers,
         Lower_95_CI = Wilson_lowerCI(PC_Presentations, 0.05, Cancers), 
...
n_cancers_route_plot <- hb_output |>
  mutate(incidence_year = as.character(incidence_year)) |>
  ggplot(aes(x = incidence_year, y = Presentations, fill = emergency_flag)) +
  facet_wrap(~hb2019name, scales = "free") +
  geom_bar(position = position_dodge(), stat = "identity") +
  scale_fill_manual("Route",
                    values = phs_colours(c("phs-green", "phs-blue"))) +
  ylab("Cancer Diagnoses") +
  xlab("Year") + 
  theme(axis.text.x = element_text(vjust = 0.5, angle = 45, size = 6.5))
